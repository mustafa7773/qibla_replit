...
(function () {
  ...
  //
  // قبل الانتقال للخادم كانت السجلات في المفتاح sky_completed_mosques_v1.
  // بعد الانتقال صارت القراءة من ذاكرة DataAPI، فبدت السجلات القديمة وكأنها
  // "مُسحت" رغم بقائها في المتصفح. هذه الدالة تنقلها مرة واحدة إلى الخادم
  // ثم تُعلّم المفتاح كمُرحَّل — بلا حذف الأصل احتياطاً.
  // ==========================================================================

  const MIGRATED_FLAG = "sky_completed_migrated_v1";

  async function migrateLegacy() {
    if (!window.DataAPI) return { ok: false, migrated: 0 };
    if (localStorage.getItem(MIGRATED_FLAG)) return { ok: true, migrated: 0 };
    ...
    // لا نرفع سجلاً موجوداً على الخادم أصلاً
    const existing = new Set(window.DataAPI.cached("completed").map((r) => r.recordId));

    let migrated = 0;
    for (const r of legacy) {
      const id = r.id || r.recordId;
      if (!id || existing.has(id)) continue;
      const res = await window.DataAPI.save("completed", {
        recordId: id,
        completionDate: r.completionDate || "",
        governorate: r.governorate || "",
        price: Number(r.price) || 0,
    ...
    if (migrated) localStorage.setItem(MIGRATED_FLAG, "1");
  ...
  async function refresh() {
    ...
    try {
      ...
      // أول تشغيل بعد التحديث: ننقل السجلات القديمة قبل القراءة
      const mig = await migrateLegacy();
      const records = await window.DataAPI.list("completed");
      return { ok: true, total: records.length, migrated: mig.migrated || 0 };
    } catch (err) {
